...
def run_benchmark(framework_name, outdir):
    ...
    exit_code = run_cmd(
        f"{python_cmd} {framework_name} benchmark constraint -o {outdir} -u {outdir} --logging debug"
    )
    return exit_code
...
def train(
    ...
    with app.app_context():
        try:
            outdir = setup_train(
                outdir, train_ids, test_ids, target, max_runtime_seconds, cores
            )
