...
def get_runbenchmark_cmd():
    ...
    if os.path.exists("/bench/venv/bin"):
        ...
        return (
            "/bench/venv/bin/python3 -W ignore /bench/automlbenchmark/runbenchmark.py"
        )
    else:
...
def on_success_train(framework_pk, outdir):
    ...
    try:
        ...
        logger.debug(
            "Searching for results files with glob: %s",
            outdir + "/*/scores/results.csv",
        )
        ...
        for model_fold, local_path in enumerate(fs):
            ...
            framework_preds = TabularFrameworkPredictions(
                framework.user_pk, framework.pk, dset.pk, model_fold, local_path
            )

            # upload the predictions
            _, ext = os.path.splitext(local_path)
            gcp_path = (
                f"tabular-framework-predictions/{framework_preds.id}/predictions{ext}"
            )
...
def runbenchmark(
    self, framework_pk, framework_name, dataset_id, constraint_id, outdir, user_dir
):
    """
...
def runbenchmark_on_failure(
    err_request, err_message, err_traceback, framework_pk, outdir
):
    """
...
def runbenchmark_predict_on_failure(
    err_request, err_message, err_traceback, preds_pk, outdir
):
